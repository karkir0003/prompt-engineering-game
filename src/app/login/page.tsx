...
export default function LoginPage() {
  ...
  const [loginState, loginAction, isLoginPending] = useActionState(signInWithPassword, null)
  const [signupState, signupAction, isSignupPending] = useActionState(signUpWithPassword, null)
      ...
      <div className="w-full max-w-sm space-y-6 rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        <form className="space-y-4">
          ...
          <div className="flex flex-col gap-2 pt-2">
            ...
            >
              {isLoginPending ? <Loader2 className="animate-spin h-4 w-4" /> : 'Log In'}
            </button>
            ...
            >
              {isSignupPending ? <Loader2 className="animate-spin h-4 w-4" /> : 'Sign Up'}
            </button>
          </div>
        </form>
      </div>
